...
class A2CAgent:
    ...
    def __init__(self, config, value_network, actor_network):
        self.config = config
...
if __name__ == '__main__':
    ...
    config_a2c = {'actor_network': {'learning_rate': 0.001}, 'entropy': 1, 'env_id': 'CartPole-v1', 'gamma': 1,
                  'seed': 1, 'value_network': {'learning_rate': 0.001}}

    env_fns = [partial(gym.make, config_a2c['env_id']) for _ in range(num_env)]
    env = SubprocVecEnv(env_fns)

    value_network = ValueNetwork(env.observation_space.shape[0], hidden_dimension, 1)
    actor_network = ActorNetwork(env.observation_space.shape[0], hidden_dimension, env.action_space.n)
    agent = A2CAgent(config_a2c, value_network, actor_network)
    rewards = agent.learn()
